...
const routeRewritePlugin = () => {
  return {
    ...
    configureServer(server) {
      ...
      const storyReadMatch = normalizedPath.match(/^\/stories\/([^/]+)\/read$/);
      if (storyReadMatch) {
        const storyId = storyReadMatch[1];
      ...
      // 3b. /stories/{storyId}/index.html -> /stories/story.html?storyId={storyId}
      if (path.match(/^\/stories\/([^/]+)\/index\.html$/)) {
        const storyId = path.match(/^\/stories\/([^/]+)\/index\.html$/)[1];
        const params = new URLSearchParams(queryString || '');
        params.set('storyId', storyId);
        req.url = `/stories/story.html?${params.toString()}`;
        process.stdout.write(`\n✅ PLUGIN REWRITE: ${method} "${originalUrl}" -> "${req.url}"\n`);
        next();
        return;
      }
      
      // 3c. /stories/{storyId} -> /stories/story.html?storyId={storyId}
      const storyDetailMatch = normalizedPath.match(/^\/stories\/([^/]+)$/);
      if (storyDetailMatch) {
        const storyId = storyDetailMatch[1];
        // Skip if it's trying to access index.html directly (already handled above)
      ...
      process.stdout.write('Routes: /child/badges, /parent/dashboard, /stories/{id}/read, /stories/{id}, /chat, /stories\n\n');
    }
  };
};
...
export default defineConfig({
  ...
  // Tell Vite to treat HTML files as static assets, not modules
  // This prevents Vite from trying to parse HTML as JavaScript during import analysis
  assetsInclude: ['**/*.html'],
  build: {
    outDir: 'dist',
    assetsDir: 'assets',
    rollupOptions: {
      input: {
        ...
        comicPhotosynthesis: resolve(__dirname, 'comic-photosynthesis.html'),
  ...
  server: {
    ...
    // Prevent HTML files from being processed as modules
  ...
  configureServer(server) {
    ...
    process.stdout.write('        /stories/{id} -> /stories/story.html?storyId={id}\n');
